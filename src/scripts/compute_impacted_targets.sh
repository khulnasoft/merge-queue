...
logIfVerbose() {
	# khulnasoft-ignore(shellcheck/SC2312): Always query date with each echo statement.
...
git -c "user.name=Khulnasoft Actions" -c "user.email=actions@khulnasoft.com" merge --squash "${PR_BRANCH}"
